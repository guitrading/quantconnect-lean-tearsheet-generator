...
class HTMLTearsheet:
    ...
    def generate(self, output_file: str = 'tearsheet.html') -> None:
        ...
        fig = self._create_figure(
            equity, returns, strat_metrics, benchmark_returns, bench_metrics,
            start_date, end_date
        )
    ...
    def _create_figure(
        self, equity, returns, strat_metrics, benchmark_returns, bench_metrics,
        start_date, end_date
    ):
    ...
    def _add_equity_curves(self, fig, equity, returns, benchmark_returns):
        ...
        fig.add_trace(
            go.Scatter(
                x=equity.index, y=equity_norm, name='Strategy',
                line=dict(color='blue', width=2),
                legendgroup='equity', legendgrouptitle_text='Cumulative Returns'
            ),
            row=2, col=1
        )
        ...
        if benchmark_returns is not None:
            ...
            fig.add_trace(
                go.Scatter(
                    x=bench_equity.index, y=bench_equity_norm, name='Benchmark',
    ...
    def _add_drawdowns(self, fig, returns, benchmark_returns):
        ...
        fig.add_trace(
            go.Scatter(
                x=strat_dd.index, y=strat_dd * 100, name='Strategy DD',
                fill='tozeroy', line=dict(color='red'),
                legendgroup='drawdown', legendgrouptitle_text='Drawdown'
            ),
            row=3, col=1
        )
        ...
        if benchmark_returns is not None:
            ...
            fig.add_trace(
                go.Scatter(
                    x=bench_dd.index, y=bench_dd * 100, name='Benchmark DD',
                    fill='tozeroy', line=dict(color='blue', width=1), opacity=0.3,
    ...
    def _add_rolling_sharpe(self, fig, returns, benchmark_returns):
        ...
        fig.add_trace(
            go.Scatter(
                x=rolling_sharpe.index, y=rolling_sharpe, name='Strategy',
                line=dict(color='blue', width=2),
                legendgroup='sharpe', legendgrouptitle_text='Rolling Sharpe'
            ),
            row=4, col=1
        )
        ...
        fig.add_trace(
            go.Scatter(
                x=rolling_sharpe.index, y=[avg_sharpe] * len(rolling_sharpe),
        ...
        if benchmark_returns is not None:
            ...
            fig.add_trace(
                go.Scatter(
                    x=bench_rolling_sharpe.index, y=bench_rolling_sharpe,
                    name='Benchmark',
                    line=dict(color='orange', width=2),
                    legendgroup='sharpe'
                ),
